 ...
 * @param {ProbeGrid|null} grid - Probe grid from buildProbeGrid (mm), or null
 * @param {Point3[]} probedPositions - Probed positions in mm
 * @param {string} units - Current units (METRIC_UNITS or IMPERIAL_UNITS)
 * @returns {Point3} Compensated point in current units
 */
const compensatePoint = (pt, grid, probedPositions, units = METRIC_UNITS) => {
  ...
  let dz = grid ? bilinearZ(grid, x, y) : null;
  if (dz === null) {
    dz = planeFitZ(x, y, probedPositions);
  }
  if (dz === null) {
...
export const applyProbeCompensation = (gcodeStr, probeData = []) => {
  ...
  // Bilinear interpolation lattice; null (and per-point plane-fit fallback) when
  // the probe data is not a rectangular grid.
    ...
    if (isAbsolutePositioning) {
      ...
      if (p0Initialized) {
        ...
        for (let i = 1; i < segments.length; i++) {
          ...
          const cpt = compensatePoint(seg, grid, probedPositions, units);
        ...
        const cpt = compensatePoint(pt, grid, probedPositions, units);
        const newLine = `${lineWithoutXYZ} X${cpt.x.toFixed(3)} Y${cpt.y.toFixed(3)} Z${cpt.z.toFixed(3)}`;
        results.push(newLine.trim());
        p0Initialized = true;
      }
