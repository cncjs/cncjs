...
const log = logger('controller:Cirqoid');
const noop = _.noop;

class CirqoidController {
    type = CIRQOID;
    ...
    addConnection(socket) {
        ...
        if (!_.isEmpty(this.settings)) {
            // controller settings
            socket.emit('controller:settings', CIRQOID, this.settings);
            socket.emit('Cirqoid:settings', this.settings); // Backward compatibility
        }
        if (!_.isEmpty(this.state)) {
            // controller state
            socket.emit('controller:state', CIRQOID, this.state);
            socket.emit('Cirqoid:state', this.state); // Backward compatibility
        }
        if (this.feeder) {
            // feeder status
...
export default CirqoidController;
