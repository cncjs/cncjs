...
import { SMOOTHIE_MACHINE_STATE_HOLD } from '../constants';
...
// distinction is observable on the 'connection:write' socket event.
const captureSerialWrites = (controller) => {
  const events = [];
  controller.sockets['test-socket'] = {
    emit: (event, ...args) => {
      if (event === 'connection:write') {
        events.push({ data: args[1], context: args[2] });
      }
    },
  };
  return events;
...
describe('SmoothieController', () => {
  describe('simple commands', () => {
    test('feed_hold writes suspend with the client write source', () => {
    ...
    test('cycle_start writes resume', () => {
      const { controller, writes } = create();
  ...
  describe('lasertest', () => {
    test('lasertest:on with no power writes the off sequence (power 0 means off)', () => {
      const { controller, writes } = create();
  ...
  describe('sender workflow', () => {
    ...
    test('gcode:stop writes resume when the active state is Hold', () => {
      ...
      controller.command('gcode:start');
      controller.command('gcode:pause');
      controller.state = { machineState: SMOOTHIE_MACHINE_STATE_HOLD };
      controller.command('gcode:stop');
  ...
  describe('deprecated aliases', () => {
    test.each([
      ...
      ['stop', 'gcode:stop', (ctl) => {
        ctl.command('gcode:load', 'gcode-test', 'G0 X1\nG0 X2');
        ctl.state = { machineState: SMOOTHIE_MACHINE_STATE_HOLD };
      }],
  ...
  describe('feeder commands', () => {
    ...
    test('feeder_start resumes the feeder and drains the queue', () => {
      const { controller, writes } = create();
    ...
    test('feeder_stop drops queued lines', () => {
      const { controller, writes } = create();
  ...
  describe('macros and watch directory', () => {
    ...
    test('watchdir:load reads the file through fs and loads it', () => {
      const { controller, writes } = create();
      const readFileSpy = jest.spyOn(fs, 'readFile').mockImplementation((file, encoding, callback) => callback(null, 'G0 X1\nG0 X2'));
      const callback = jest.fn();
    ...
    test('watchdir:load forwards fs read errors to the callback', () => {
      const { controller, writes } = create();
      jest.spyOn(fs, 'readFile').mockImplementation((file, encoding, callback) => callback(new Error('read failed')));
      const callback = jest.fn();
