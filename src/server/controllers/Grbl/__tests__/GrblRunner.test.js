...
describe('GrblRunner', () => {
  ...
  test('GrblLineParserResultStatus: v1.1 A accessory state clears on override refresh', () => {
    const runner = new GrblRunner();
  ...
  test('GrblLineParserResultAlarm: sets machineState', () => {
    return new Promise((resolve) => {
      const runner = new GrblRunner();
      runner.on('alarm', () => {
        expect(runner.state.status.machineState).toEqual('Alarm');
        resolve();
      });
