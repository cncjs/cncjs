...
test('resolved', (t) => {
  ...
  t.matchOnly(res, [1, 2, 49, 800, 555]);
  t.end();
});
...
test('boolean', (t) => {
  ...
  t.matchOnly(evaluateExpression(src), [true, true, true, true]);
  t.end();
});

test('array methods', (t) => {
  const src = '[1, 2, 3].map(function(n) { return n * 2 })';
  t.matchOnly(evaluateExpression(src), [2, 4, 6]);
  t.end();
});

test('array methods with vars', (t) => {
  const src = '[1, 2, 3].map(function(n) { return n * x })';
  t.matchOnly(evaluateExpression(src, { x: 2 }), [2, 4, 6]);
  t.end();
});
