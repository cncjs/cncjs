...
test('GrblRunner', (t) => {
  ...
  // RX Buffer        | 8
  // Limit Pins       | 16
  t.test('GrblLineParserResultStatus: all zeroes in the mask ($10=0)', (t) => {
  ...
  t.test('GrblLineParserResultStatus: default ($10=3)', (t) => {
    const runner = new GrblRunner();
    runner.on('status', ({ raw, ...status }) => {
  ...
  t.test('GrblLineParserResultStatus: 6-axis', (t) => {
    const runner = new GrblRunner();
    runner.on('status', ({ raw, ...status }) => {
  ...
  t.test('GrblLineParserResultStatus: set all bits to 1 ($10=31)', (t) => {
    const runner = new GrblRunner();
    runner.on('status', ({ raw, ...status }) => {
  ...
  t.test('GrblLineParserResultOk', (t) => {
    const runner = new GrblRunner();
    runner.on('ok', ({ raw }) => {
      t.equal(raw, 'ok');
      t.end();
  ...
  t.test('GrblLineParserResultError', (t) => {
    const runner = new GrblRunner();
  ...
  t.test('GrblLineParserResultParameters:G54,G55,G56,G57,G58,G59,G28,G30,G92', (t) => {
    const lines = [
  ...
  t.test('GrblLineParserResultParameters:TLO', (t) => {
    const runner = new GrblRunner();
    runner.on('parameters', ({ name, value, raw }) => {
  ...
  t.test('GrblLineParserResultParameters:PRB', (t) => {
    const runner = new GrblRunner();
    runner.on('parameters', ({ name, value, raw }) => {
  ...
  t.test('GrblLineParserResultFeedback', (t) => {
    const lines = [
      // $I - View build info
      '[0.9j.20160303:]',
  ...
  t.test('GrblLineParserResultSettings', (t) => {
    const lines = [
  ...
  t.test('GrblLineParserResultStartup', (t) => {
    t.test('Grbl 0.9j', (t) => {
      const runner = new GrblRunner();
